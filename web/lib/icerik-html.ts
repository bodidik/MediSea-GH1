  ...
  while ((esl = ETIKET.exec(html))) {
    ...
    if (kapanis) {
      ...
      for (let i = yigin.length - 1; i >= 0; i--) {
        if (yigin[i] !== ad) continue;
        /* ÜST ETİKET KAPANIYOR, İÇİNDE AÇIK BİÇİM ÖGESİ VAR: `<p><strong>a</p>`.
           Tarayıcı `</p>`de `strong`u yığından düşürür ama "etkin biçimlendirme"
           listesinde tutar ve SONRAKİ metinde yeniden üretir — sızıntının ta
           kendisi. Dalın ilk sürümü bu biçimi görmüyordu (16 Eyl 2026 ölçümü:
           2436 bölümde tek sızan bölüm `miyeloproliferatif#0`, onarıcı ondan
           sonra da sızıyordu). Kapanışları üst etiketten ÖNCE yaz. */
        const ek = yigin.slice(i + 1).filter((x) => BICIMLENDIRME.has(x)).reverse().map((x) => `</${x}>`).join("");
        if (ek) {
