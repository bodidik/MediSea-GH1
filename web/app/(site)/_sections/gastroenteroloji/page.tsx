...
const BASE = "/tr/topics/gastroenteroloji/";
...
async function getItems(): Promise<Item[]> {
  ...
  // 2) Öncelik: Backend’den dinamik liste (kamu içerik → ISR)
  try {
    const backend = backendBase();
    const url = new URL("/api/topics", backend);
    url.searchParams.set("section", "gastroenteroloji");
    ...
    if (r.ok) {
      ...
      if (arr.length > 0) {
        const items = arr
          ...
          .sort((a: Item, b: Item) => a.title.localeCompare(b.title, "tr"));
...
export default async function Page() {
  ...
  return (
    <main className="max-w-6xl mx-auto p-6">
 	...
 	<Link
        href="/tr/topics/gastroenteroloji"
        className="inline-flex items-center border rounded-lg px-3 py-2 bg-white hover:shadow-sm"
      >
        Gastroenteroloji konularını listele →
      </Link>

      <Link
      href="/tr/sections"
      className="inline-flex items-center border rounded-lg px-3 py-2 bg-white hover:shadow-sm opacity-80"
      >
       ← Tüm bölümler
     </Link>
   </div>
