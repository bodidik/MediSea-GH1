...
export default function GastroDashboard({ topics }: { topics: Topic[] }) {
  ...
  useEffect(() => {
    setIsClient(true);
    const saved = localStorage.getItem("gastro_favorites");
    if (saved) {
      try {
  ...
  const toggleFavorite = (e: React.MouseEvent, slug: string) => {
    ...
    localStorage.setItem("gastro_favorites", JSON.stringify(newFavs));
  };
      ...
      <div className="sticky top-16 z-40 bg-gray-50/95 backdrop-blur py-4 border-b border-gray-200 mb-8 -mx-4 px-4 sm:mx-0 sm:px-0 shadow-sm transition-all">
        <div className="flex flex-col gap-4">
          ...
          <div className="relative w-full">
            <input
              type="text"
              placeholder="Hastalık, semptom veya tanı ara..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-11 pr-4 py-3 rounded-xl border border-gray-200 focus:border-orange-500 focus:ring-4 focus:ring-orange-100 outline-none transition-all text-gray-800 placeholder-gray-400 font-medium"
            />
            {/* Sabit Boyutlu SVG */}
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {filteredTopics.length > 0 ? (
          filteredTopics.map((t) => {
            ...
            return (
              <Link key={t.slug} href={`/tr/topics/gastroenteroloji/${t.slug}`} className="block group h-full">
                <div className="bg-white border border-gray-200 rounded-xl p-5 hover:border-orange-400 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 h-full flex flex-col relative">
                  
                  {/* ❤️ FAVORİ BUTONU (Geliştirilmiş Görsellik) */}
                  <button
                    ...
                    className={`absolute top-4 right-4 z-20 p-2 rounded-full transition-all duration-200 active:scale-95 ${
                      isFav 
                        ? "text-red-500 bg-red-50 hover:bg-red-100 shadow-sm"  // DOLU HALİ
                        : "text-gray-300 bg-transparent hover:bg-gray-50 hover:text-gray-400" // BOŞ HALİ
                    }`}
                    title={isFav ? "Favorilerden Çıkar" : "Favorilere Ekle"}
                  >
                    {/* SVG Mantığı: isFav ise 'fill-current' (dolu), değilse 'fill-none' (boş) */}
                    <svg 
                      width="24" 
                      height="24" 
                      className={`w-6 h-6 transition-colors ${isFav ? "fill-current stroke-none" : "fill-none stroke-current stroke-2"}`} 
                      viewBox="0 0 24 24"
                    >
                  ...
                  {/* KART İÇERİĞİ */}
                  <div className="text-[10px] font-bold text-orange-400 uppercase tracking-wider mb-2 pr-10">
                    {t.subcategory || "Genel"}
                  </div>

                  <h3 className="font-bold text-gray-800 group-hover:text-orange-700 text-lg leading-snug mb-4">
                    {t.title}
                  </h3>
          ...
          <div className="col-span-full text-center py-20 bg-white rounded-xl border border-dashed border-gray-200">
             {selectedCategory === "❤️ Favorilerim" ? (
              <>
                ...
                <p className="text-sm text-gray-400 mt-2">Kartlardaki kalp ikonuna tıklayarak ekleyebilirsiniz.</p>
                <button onClick={() => setSelectedCategory("Tümü")} className="mt-6 px-6 py-2 bg-orange-50 text-orange-700 rounded-full font-bold hover:bg-orange-100 transition-colors">
                  Tüm Konuları Gör
                </button>
              </>
            ) : (
              <>
                ...
                <button onClick={() => {setSearchQuery(""); setSelectedCategory("Tümü")}} className="text-orange-600 font-bold hover:underline">
                  Temizle
                </button>
              </>
            )}
