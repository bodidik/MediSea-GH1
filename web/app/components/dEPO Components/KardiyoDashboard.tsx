...
export default function KardiyoDashboard({ topics }: { topics: Topic[] }) {
  const [selectedCategory, setSelectedCategory] = useState("Tümü");
  const [searchQuery, setSearchQuery] = useState("");
  
  // FAVORİLER (Kardiyoloji'ye özel hafıza)
  const [favorites, setFavorites] = useState<string[]>([]);
  const [isClient, setIsClient] = useState(false);

  useEffect(() => {
    setIsClient(true);
    const saved = localStorage.getItem("kardiyo_favorites"); // <-- ÖNEMLİ: Anahtar farklı
    if (saved) {
      try {
  ...
  const toggleFavorite = (e: React.MouseEvent, slug: string) => {
    ...
    localStorage.setItem("kardiyo_favorites", JSON.stringify(newFavs));
  };
      ...
      <div className="sticky top-16 z-40 bg-gray-50/95 backdrop-blur py-4 border-b border-gray-200 mb-8 -mx-4 px-4 sm:mx-0 sm:px-0 shadow-sm transition-all">
        <div className="flex flex-col gap-4">
          ...
          <div className="relative w-full">
            <input
              type="text"
              placeholder="MI, aritmi, hipertansiyon veya ilaç ara..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-11 pr-4 py-3 rounded-xl border border-gray-200 focus:border-red-500 focus:ring-4 focus:ring-red-100 outline-none transition-all text-gray-800 placeholder-gray-400 font-medium"
            />
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {filteredTopics.length > 0 ? (
          filteredTopics.map((t) => {
            ...
            return (
              <Link key={t.slug} href={`/tr/topics/kardiyoloji/${t.slug}`} className="block group h-full">
                <div className="bg-white border border-gray-200 rounded-xl p-5 hover:border-red-400 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 h-full flex flex-col relative">
                  ...
                  <div className="text-[10px] font-bold text-red-500 uppercase tracking-wider mb-2 pr-10">
                    {t.subcategory || "Genel"}
                  </div>

                  <h3 className="font-bold text-gray-800 group-hover:text-red-700 text-lg leading-snug mb-4">
                    {t.title}
                  </h3>
          ...
          <div className="col-span-full text-center py-20 bg-white rounded-xl border border-dashed border-gray-200">
             {selectedCategory === "❤️ Favorilerim" ? (
              <>
                ...
                <button onClick={() => setSelectedCategory("Tümü")} className="mt-6 px-6 py-2 bg-red-50 text-red-700 rounded-full font-bold hover:bg-red-100 transition-colors">
                  Tüm Konuları Gör
                </button>
              </>
            ) : (
              <>
                ...
                <button onClick={() => {setSearchQuery(""); setSelectedCategory("Tümü")}} className="text-red-600 font-bold hover:underline">
                  Temizle
                </button>
              </>
            )}
